...
ARTIFACT_DIR="${ANDROID_OPENMMO_ARTIFACT_DIR:-artifacts/android-openmmo-runtime}"
DRIVER="${ANDROID_OPENMMO_ACCEPTANCE_DRIVER:-scripts/ci/android_openmmo_runtime_acceptance.py}"

mkdir -p "$TERRAIN_DIR" "$ARTIFACT_DIR"
...
cleanup() {
  ...
  cp "$SERVER_LOG" "$ARTIFACT_DIR/server.stdout.log" 2>/dev/null || true
  cp "$SERVER_ERR" "$ARTIFACT_DIR/server.stderr.log" 2>/dev/null || true
}
trap cleanup EXIT
...
python3 "$DRIVER"   --token "$TOKEN"   --server "ws://10.0.2.2:10006"   --server-log "$SERVER_LOG"   --server-pid "$SERVER_PID"
